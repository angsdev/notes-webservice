    ...
    <body>
      <div class="flex items-center justify-center min-h-screen p-5 bg-gray-600 min-w-screen">
        <div class="max-w-xl p-8 text-center text-gray-800 bg-white shadow-xl lg:max-w-3xl rounded-3xl lg:p-12">
          <h3 class="text-2xl">Thanks for being part of ${process.env.APP_NAME}</h3>
          ...
          <div class="mt-4">
            <a class="px-2 py-2 text-blue-200 bg-blue-600 rounded" href="${process.env.SERVER_HOST}:${process.env.SERVER_PORT}/${data.version}/auth/email/verify/${data.token}" target="_blank">Click to Verify Email</a>
            <p class="mt-4 text-sm">
              If you’re having trouble clicking the "Verify Email Address" button, copy and paste the URL below into your web browser:
              <a href="#" class="text-blue-600">${process.env.SERVER_HOST}:${process.env.SERVER_PORT}/${data.version}/auth/email/verify/${data.token}</a>
            </p>
          </div>
        </div>
      </div>
